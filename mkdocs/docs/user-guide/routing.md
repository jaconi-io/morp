...
Routing in MORP uses [Spring Cloud Gateway](https://docs.spring.io/spring-cloud-gateway/docs/current/reference/html/) in WebMVC mode.
A basic route could be configured like this:
...
| `TenantFromHost`   | <pre>TenantFromHost=foo,static.example.com,another-static.example.com</pre> | static.example.com            | foo            |
| `TenantFromHeader` | <pre>TenantFromHeader=X-Tenant-ID</pre>                            | X-Tenant-ID: foo              | foo            |

Predicates are configured in the `spring.cloud.gateway.server.webmvc.routes[*].predicates` section and are applied per
